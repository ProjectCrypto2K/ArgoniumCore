...
cd src\release
zip ..\..\..\..\builds\%ACHIVE_NAME_PREFIX%%version%.zip argoniumcd.exe simplewallet.exe simpleminer.exe
IF %ERRORLEVEL% NEQ 0 (
  goto error
)
...
msbuild src/qt-argoniumc.vcxproj  /p:Configuration=Release /t:Build
...
cd src\release
zip ..\..\..\..\builds\%ACHIVE_NAME_PREFIX%%version%.zip qt-argoniumc.exe
